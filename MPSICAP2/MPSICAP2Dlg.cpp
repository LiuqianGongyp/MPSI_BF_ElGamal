...
CMPSICAP2Dlg::CMPSICAP2Dlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_MPSICAP2_DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CMPSICAP2Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	//  DDX_Control(pDX, IDC_OpenFile, m_path);
...
BEGIN_MESSAGE_MAP(CMPSICAP2Dlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_OpenFile, &CMPSICAP2Dlg::OnBnClickedOpenfile)
	ON_BN_CLICKED(IDC_Begin, &CMPSICAP2Dlg::OnBnClickedBegin)
END_MESSAGE_MAP()


// CMPSICAP2Dlg 消息处理程序

BOOL CMPSICAP2Dlg::OnInitDialog()
{
...
void CMPSICAP2Dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR CMPSICAP2Dlg::OnQueryDragIcon()
{
...
void CMPSICAP2Dlg::OnBnClickedOpenfile()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CMPSICAP2Dlg::OnBnClickedBegin()
{
	// TODO: 在此添加控件通知处理程序代码
	clock_t TStart, TEnd;
	double RunTime = 0;
	...
	//response
	long long SizeX1;
	ReceiveBits(party.ciphertext1, IPADdress[Me - 1], &SizeX1);
	ReceiveBits(party.ciphertext2, IPADdress[Me - 1], &SizeX1);
	unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();//种子
	std::shuffle(party.ciphertext1, party.ciphertext1 + SizeX1, std::default_random_engine(seed));
	std::shuffle(party.ciphertext2, party.ciphertext2 + SizeX1, std::default_random_engine(seed));
	for (int j = 0; j < SizeX1; j++) {
		ZZ c0_1, c0_2;
		ZZ m0(0);
		TELEnc(p, q, g, PK, m0, &c0_1, &c0_2);
		party.ciphertext1[j] = MulMod(party.ciphertext1[j], c0_1, p);
		party.ciphertext2[j] = MulMod(party.ciphertext2[j], c0_2, p);
	}
	bitarray1 = strdup(zToString(party.ciphertext1, (ZZ)SizeX1).c_str());
	bitarray2 = strdup(zToString(party.ciphertext2, (ZZ)SizeX1).c_str());
	SendBitString(bitarray1, (ZZ)SizeX1);
	SendBitString(bitarray2, (ZZ)SizeX1);
	ReceiveBits(party.ciphertext1, IPADdress[NumParty - 1], &SizeX1);
	//computation
	for (int j = 0; j < SizeX1; j++) {
